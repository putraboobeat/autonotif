...
const TicketModel = {
   ...
   * Mengikuti perubahan departemen (kantor_pertanahan) terbaru yang di-scrape dari OCA.
   */
  updateInfo(ticket) {
    const db = getDb();
    ...
    // Perbarui kantor pertanahan mengikuti data terbaru dari OCA, 
    // namun gunakan yang lama sebagai fallback jika OCA mengembalikan kosong
    const finalKantor = (ticket.kantorPertanahan || '').trim() !== '' ? ticket.kantorPertanahan : existingKantor;
...
const IgPostModel = {
  ...
  save({ shortcode, link, caption, matched_code, notified_group, status, error_msg, post_date, image_url }) {
    const db = getDb();
    const stmt = db.prepare(`
      INSERT INTO processed_ig_posts 
      (shortcode, link, caption, matched_code, notified_group, status, error_msg, post_date, image_url)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
